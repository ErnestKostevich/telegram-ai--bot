...
Исправлено: единый контекст для текста/фото/голоса/документов
"""

import os, json, logging, random, asyncio, signal, io, base64, tempfile
...
class UnifiedContext:
    ...
    def add_user_message(self, user_id: int, content: Any):
        """Добавить сообщение пользователя (текст, [текст, Image], и т.д.)"""
    ...
    def get_text_history(self, user_id: int) -> List[Dict]:
        """История только с текстом (для Gemini chat)"""
        history = self.get_history(user_id)
        result = []
        for i, msg in enumerate(history):
            # Из старых сообщений убираем картинки
            if i < len(history) - 4:
...
def t(key: str, lang: str, **kw) -> str:
    ...
    return L.get(lang, L['ru']).get(key, L['ru'].get(key, key)).format(**kw) if kw else L.get(lang, L['ru']).get(key, key)
...
if DATABASE_URL:
    try:
        ...
        # Миграция: добавляем language если нет
        try:
            insp = inspect(engine)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ИСПРАВЛЕНО: Фото добавляется в ЕДИНЫЙ контекст"""
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ИСПРАВЛЕНО: Голос транскрибируется и добавляется в ЕДИНЫЙ контекст"""
    uid = update.effective_user.id
    lang = get_lang(uid)
    ...
    try:
        ...
        # ✅ Добавляем в ЕДИНЫЙ контекст как текст
        unified_ctx.add_user_message(uid, f"[Голосовое]: {transcription}")
        
        # Получаем историю и отправляем
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ИСПРАВЛЕНО: Документ добавляется в ЕДИНЫЙ контекст"""
    uid = update.effective_user.id
    lang = get_lang(uid)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ИСПРАВЛЕНО: Текст добавляется в ЕДИНЫЙ контекст"""
    ...
    menu_map = {
        t('menu_chat', 'ru'): 'chat', t('menu_chat', 'en'): 'chat',
        t('menu_notes', 'ru'): 'notes', t('menu_notes', 'en'): 'notes',
        t('menu_weather', 'ru'): 'weather', t('menu_weather', 'en'): 'weather',
        t('menu_time', 'ru'): 'time', t('menu_time', 'en'): 'time',
        t('menu_games', 'ru'): 'games', t('menu_games', 'en'): 'games',
        t('menu_info', 'ru'): 'info', t('menu_info', 'en'): 'info',
        t('menu_vip', 'ru'): 'vip', t('menu_vip', 'en'): 'vip',
        t('menu_gen', 'ru'): 'gen', t('menu_gen', 'en'): 'gen',
        t('menu_admin', 'ru'): 'admin', t('menu_admin', 'en'): 'admin',
    }
...
async def clear_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    unified_ctx.clear(uid)  # ✅ Очищаем ЕДИНЫЙ контекст
...
async def ai_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        ...
        return
    # Используем handle_message логику
...
async def random_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(t('random_result', lang, min=1, max=100, r=random.randint(1, 100)), parse_mode=ParseMode.HTML)
...
# === ЧАСТЬ 3: НАПОМИНАНИЯ, АДМИН, CALLBACKS, MAIN ===
