...
async def keep_bot_active(bot):
    global CREATOR_ID
    if CREATOR_ID is None:
        return
    try:
        ping_msg = await bot.send_message(chat_id=CREATOR_ID, text="Keep-alive ping", disable_notification=True)
        pong_msg = await bot.send_message(chat_id=CREATOR_ID, text="Keep-alive pong", reply_to_message_id=ping_msg.message_id, disable_notification=True)
        await asyncio.sleep(1)
        await bot.delete_message(chat_id=CREATOR_ID, message_id=ping_msg.message_id)
        await bot.delete_message(chat_id=CREATOR_ID, message_id=pong_msg.message_id)
        logger.info("Keep-alive messages sent and deleted.")
    except Exception as e:
        logger.error(f"Keep-bot-active error: {e}")
...
def main():
    ...
    scheduler.add_job(keep_bot_active, 'interval', minutes=10, args=[application.bot])
