...
# 2. Замените инициализацию Gemini
generation_config = {
    "temperature": 1,
    "top_p": 0.95,
    "top_k": 40,
...
# 3. Добавьте загрузку вашей модели
logger.info("🚀 Загружаю вашу модель с HuggingFace...")
...
try:
    ...
    logger.info("✅ Ваша модель загружена успешно!")
    MODEL_LOADED = True
...
# 4. Добавьте функцию генерации
def generate_with_your_model(prompt: str, max_new_tokens: int = 512) -> str:
    if not MODEL_LOADED or your_model is None:
        return "❌ Моя модель временно недоступна. Попробуйте позже."
    
    try:
        formatted_prompt = f"""<|begin_of_text|><|start_header_id|>system<|end_header_id|>Ты — AI DISCO BOT, очень умный и дружелюбный ассистент, созданный Ernest Kostevich. Отвечай на том языке, на котором задан вопрос. Будь полезным, вежливым и информативным.<|eot_id|><|start_header_id|>user<|end_header_id|>{prompt}<|eot_id|><|start_header_id|>assistant<|end_header_id|>"""
...
# --- СТРОКИ ЛОКАЛИЗАЦИИ ---
# (УДАЛЕНО все связанное с генерацией изображений)
localization_strings = {
    'ru': {
        'welcome': (
            "🤖 <b>AI DISCO BOT</b>\n\n"
            "Привет, {first_name}! Я бот на <b>Custom Llama 3.1</b>.\n\n"
            "<b>🎯 Возможности:</b>\n"
            "💬 AI-чат\n"
            "📝 Заметки и задачи\n"
            "🌍 Погода и время\n"
    ...
    'en': {
        'welcome': (
            "🤖 <b>AI DISCO BOT</b>\n\n"
            "Hi, {first_name}! I'm a bot powered by <b>Custom Llama 3.1</b>.\n\n"
            "<b>🎯 Features:</b>\n"
            "💬 AI chat\n"
            "📝 Notes and To-Dos\n"
            "🌍 Weather and Time\n"
    ...
    'it': {
        'welcome': (
            "🤖 <b>AI DISCO BOT</b>\n\n"
            "Ciao, {first_name}! Sono un bot basato su <b>Custom Llama 3.1</b>.\n\n"
            "<b>🎯 Funzionalità:</b>\n"
            "💬 Chat AI\n"
            "📝 Note e Impegni\n"
            "🌍 Meteo e Ora\n"
...
class DataStorage:
    ...
    def get_chat_session(self, user_id: int):
        # В текущей реализации с Llama история чатов обрабатывается иначе (или пока не сохраняется в сессии)
        # Оставляем заглушку или убираем зависимость от chat_sessions, если используем HF модель напрямую без истории
        return None 
...
async def analyze_image_with_gemini(image_bytes: bytes, prompt: str = "Опиши подробно что изображено") -> str:
    if vision_model is None:
        return "❌ Vision модель недоступна (нет API ключа)."
    try:
...
async def transcribe_audio_with_gemini(audio_bytes: bytes) -> str:
    if vision_model is None:
         return "❌ Vision модель недоступна (нет API ключа для транскрипции)."
    try:
        with tempfile.NamedTemporaryFile(suffix='.ogg', delete=False) as temp_file:
            temp_file.write(audio_bytes)
            temp_path = temp_file.name
        uploaded_file = genai.upload_file(path=temp_path, mime_type="audio/ogg")
        # Используем vision_model (2.5 flash) так как он мультимодальный
        response = vision_model.generate_content(["Транскрибируй это аудио:", uploaded_file])
        os.remove(temp_path)
        return response.text
    except Exception as e:
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        response_text = generate_with_your_model(analysis_prompt)
        
        storage.save_chat(user_id, f"Файл {file_name}", response_text)
        await update.message.reply_text(get_text('file_analyzing', lang, filename=file_name, text=response_text), parse_mode=ParseMode.HTML)
    except Exception as e:
...
# 5. В функции process_ai_message замените:
async def process_ai_message(update: Update, text: str, user_id: int, lang: str):
    try:
        ...
        # НОВОЕ:
        response_text = generate_with_your_model(text)
...
async def translate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        response_text = generate_with_your_model(prompt)
        await send_long_message(update.message, response_text)
    except Exception as e:
...
def main():
    ...
    logger.info("🤖 Модель: Custom Llama 3.1 + Gemini Vision")
    logger.info("🗄️ БД: " + ("PostgreSQL ✓" if engine else "Local JSON"))
    logger.info("🖼️ Генерация изображений: ОТКЛЮЧЕНА")
