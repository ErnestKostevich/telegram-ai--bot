...
generation_config = {"temperature": 0.9, "top_p": 0.95, "top_k": 40, "max_output_tokens": 2048}
safety_settings = [
...
model = genai.GenerativeModel(
    ...
    system_instruction="You are AI DISCO BOT, a friendly and helpful AI assistant. Respond in a friendly, engaging manner. Your creator is @Ernest_Kostevich."
)
    ...
    def get_user(self, user_id: int) -> Dict:
        if user_id not in self.users:
            self.users[user_id] = {
                ...
                'notes': [], 'memory': {}, 'reminders': [], 'registered': datetime.now().isoformat(),
                'last_active': datetime.now().isoformat(), 'messages_count': 0, 'commands_count': 0
            }
            self.save_users()
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    storage.update_user(user.id, {'username': user.username or '', 'first_name': user.first_name or '', 'commands_count': user_data['commands_count'] + 1})
    welcome_text = f"""🤖 <b>Добро пожаловать в AI DISCO BOT!</b>
...
async def uptime_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    days, hours, minutes, seconds = uptime.days, uptime.seconds // 3600, (uptime.seconds % 3600) // 60, uptime.seconds % 60
...
async def memory_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user['memory']:
        ...
        return
    memory_text = "🧠 <b>Ваша память:</b>\n\n" + '\n'.join([f"🔑 <b>{k}</b>: <code>{v}</code>" for k, v in user['memory'].items()])
...
async def note_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = storage.get_user(update.effective_user.id)
    user['notes'].append({'text': note_text, 'created': datetime.now().isoformat()})
    storage.save_users()
    await update.message.reply_text(f"✅ Заметка #{len(user['notes'])} сохранена!\n\n📝 {note_text}")
...
async def time_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    timezones = {'moscow': 'Europe/Moscow', 'london': 'Europe/London', 'new york': 'America/New_York', 'tokyo': 'Asia/Tokyo', 'paris': 'Europe/Paris', 'berlin': 'Europe/Berlin', 'dubai': 'Asia/Dubai', 'sydney': 'Australia/Sydney', 'los angeles': 'America/Los_Angeles', 'beijing': 'Asia/Shanghai'}
...
async def weather_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if weather_data and 'current_condition' in weather_data:
            ...
            temp_c, feels_like, description, humidity, wind_speed = current['temp_C'], current['FeelsLikeC'], current['weatherDesc'][0]['value'], current['humidity'], current['windspeedKmph']
            weather_emojis = {'Sunny': '☀️', 'Clear': '🌙', 'Partly cloudy': '⛅', 'Cloudy': '☁️', 'Overcast': '☁️', 'Mist': '🌫️', 'Patchy rain possible': '🌦️', 'Light rain': '🌧️', 'Moderate rain': '🌧️', 'Heavy rain': '⛈️', 'Patchy snow possible': '🌨️', 'Light snow': '❄️', 'Moderate snow': '❄️', 'Heavy snow': '❄️'}
            emoji = weather_emojis.get(description, '🌤️')
            weather_text = f"""{emoji} <b>Погода в {city.title()}</b>
...
async def translate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 2:
        ...
        return
    try:
        target_lang, text = context.args[0], ' '.join(context.args[1:])
...
async def random_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        min_val, max_val = (int(context.args[0]), int(context.args[1])) if len(context.args) >= 2 else (1, 100)
...
async def dice_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dice_emoji = ['⚀', '⚁', '⚂', '⚃', '⚄', '⚅'][result - 1]
    await update.message.reply_text(f"🎲 Бросаем кубик...\n\n{dice_emoji} Выпало: <b>{result}</b>", parse_mode=ParseMode.HTML)

async def coin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    result = random.choice(['Орёл', 'Решка'])
    emoji = '🦅' if result == 'Орёл' else '💰'
    await update.message.reply_text(f"🪙 Подбрасываем монету...\n\n{emoji} Выпало: <b>{result}</b>", parse_mode=ParseMode.HTML)

async def joke_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    jokes = ["Программист ложится спать. Жена говорит: — Дорогой, закрой окно, на улице холодно! Программист: — И что, если я закрою окно, на улице станет тепло? 😄", "— Почему программисты путают Хэллоуин и Рождество? — Потому что 31 OCT = 25 DEC! 🎃🎄", "Зачем программисту очки? Чтобы лучше C++! 👓"]
    await update.message.reply_text(f"😄 <b>Шутка дня:</b>\n\n{random.choice(jokes)}", parse_mode=ParseMode.HTML)

async def quote_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    quotes = ["Единственный способ сделать великую работу — любить то, что вы делаете. — Стив Джобс", "Программирование — это искусство превращать кофе в код. — Неизвестный автор"]
    await update.message.reply_text(f"💭 <b>Цитата:</b>\n\n<i>{random.choice(quotes)}</i>", parse_mode=ParseMode.HTML)

async def fact_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    facts = ["🌍 Земля — единственная планета Солнечной системы, названная не в честь бога.", "🐙 У осьминогов три сердца и голубая кровь.", "🍯 Мёд — единственный продукт питания, который не портится тысячи лет."]
...
async def vip_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if storage.is_vip(user_id):
        ...
        vip_text += f"⏰ <b>Активен до:</b> {datetime.fromisoformat(user['vip_until']).strftime('%d.%m.%Y %H:%M')}\n\n" if user['vip_until'] else "⏰ <b>Активен:</b> Навсегда ♾️\n\n"
...
async def remind_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not storage.is_vip(update.effective_user.id):
    ...
    if len(context.args) < 2:
        ...
        return
    try:
        minutes, text = int(context.args[0]), ' '.join(context.args[1:])
        remind_time = datetime.now() + timedelta(minutes=minutes)
        user = storage.get_user(update.effective_user.id)
        user['reminders'].append({'text': text, 'time': remind_time.isoformat(), 'created': datetime.now().isoformat()})
        storage.save_users()
        scheduler.add_job(send_reminder, 'date', run_date=remind_time, args=[context.bot, update.effective_user.id, text])
...
async def grant_vip_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        ...
        return
    try:
        identifier, duration = context.args[0], context.args[1].lower()
...
async def revoke_vip_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not target_id:
            ...
            return
        user = storage.get_user(target_id)
        user['vip'], user['vip_until'] = False, None
        storage.save_users()
        username_info = f"@{user['username']}" if user.get('username') else ""
...
async def backup_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_document(document=open(backup_filename, 'rb'), caption=f"✅ Резервная копия создана!\n\n📅 {datetime.now().strftime('%d.%m.%Y %H:%M')}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    identify_creator(update.effective_user)
    user_id, chat_type, text = update.effective_user.id, update.message.chat.type, update.message.text
    user = storage.get_user(user_id)
    storage.update_user(user_id, {'messages_count': user['messages_count'] + 1, 'username': update.effective_user.username or '', 'first_name': update.effective_user.first_name or ''})
...
async def handle_menu_button(update: Update, context: ContextTypes.DEFAULT_TYPE, button: str):
    user_id = update.effective_user.id
    if button == "💬 AI Чат":
        await update.message.reply_text("🤖 <b>AI Чат режим</b>\n\nПросто напиши мне что-нибудь, и я отвечу!\nИспользуй /clear чтобы очистить контекст.", parse_mode=ParseMode.HTML)
    elif button == "📝 Заметки":
        keyboard = [[InlineKeyboardButton("➕ Создать заметку", callback_data="note_create")], [InlineKeyboardButton("📋 Мои заметки", callback_data="note_list")]]
        await update.message.reply_text("📝 <b>Система заметок</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(keyboard))
    elif button == "🌍 Погода":
        await update.message.reply_text("🌍 <b>Погода</b>\n\nИспользуй: /weather [город]\nПример: /weather London", parse_mode=ParseMode.HTML)
    elif button == "⏰ Время":
        await update.message.reply_text("⏰ <b>Текущее время</b>\n\nИспользуй: /time [город]\nПример: /time Tokyo", parse_mode=ParseMode.HTML)
    elif button == "🎲 Развлечения":
        keyboard = [[InlineKeyboardButton("🎲 Кубик", callback_data="game_dice"), InlineKeyboardButton("🪙 Монета", callback_data="game_coin")], [InlineKeyboardButton("😄 Шутка", callback_data="game_joke"), InlineKeyboardButton("💭 Цитата", callback_data="game_quote")], [InlineKeyboardButton("🔬 Факт", callback_data="game_fact")]]
        ...
        if is_creator(user_id):
            keyboard = [[InlineKeyboardButton("👥 Пользователи", callback_data="admin_users")], [InlineKeyboardButton("📊 Статистика", callback_data="admin_stats")]]
...
def main():
    ...
    bot_app.add_handler(CommandHandler("uptime", uptime_command))
    bot_app.add_handler(CommandHandler("ai", ai_command))
    bot_app.add_handler(CommandHandler("clear", clear_command))
    bot_app.add_handler(CommandHandler("memorysave", memory_save_command))
    bot_app.add_handler(CommandHandler("memoryget", memory_get_command))
    bot_app.add_handler(CommandHandler("memorylist", memory_list_command))
    bot_app.add_handler(CommandHandler("memorydel", memory_del_command))
    bot_app.add_handler(CommandHandler("note", note_command))
    bot_app.add_handler(CommandHandler("notes", notes_command))
    bot_app.add_handler(CommandHandler("delnote", delnote_command))
    bot_app.add_handler(CommandHandler("time", time_command))
    ...
    bot_app.add_handler(CommandHandler("dice", dice_command))
    bot_app.add_handler(CommandHandler("coin", coin_command))
    bot_app.add_handler(CommandHandler("joke", joke_command))
    bot_app.add_handler(CommandHandler("quote", quote_command))
    bot_app.add_handler(CommandHandler("fact", fact_command))
    bot_app.add_handler(CommandHandler("vip", vip_command))
    bot_app.add_handler(CommandHandler("remind", remind_command))
    bot_app.add_handler(CommandHandler("reminders", reminders_command))
    bot_app.add_handler(CommandHandler("grant_vip", grant_vip_command))
    bot_app.add_handler(CommandHandler("revoke_vip", revoke_vip_command))
    ...
    if WEBHOOK_URL:
        logger.info(f"Starting webhook mode: {WEBHOOK_URL}/{BOT_TOKEN}")
        bot_app.run_webhook(listen='0.0.0.0', port=PORT, url_path=BOT_TOKEN, webhook_url=f"{WEBHOOK_URL}/{BOT_TOKEN}")
    else:
        logger.info("Starting polling mode (set WEBHOOK_URL to use webhook)")
