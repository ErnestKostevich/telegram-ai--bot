...
def is_creator(user_id: int) -> bool:
    ...
    if user.get('username') == CREATOR_USERNAME:
        CREATOR_ID = user_id
        logger.info(f"Creator identified by username: {user_id}")
        return True
    return False
...
async def joke_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    jokes = ["Программист ложится спать. Жена говорит: — Дорогой, закрой окно, на улице холодно! Программист: — И что, если я закрою окно, на улице станет тепло? 😄", "— Почему программисты путают Хэллоуин и Рождество? — Потому что 31 OCT = 25 DEC! 🎃🎄", "Зачем программисту очки? Чтобы лучше C++! 👓", "Искусственный интеллект никогда не заменит человека. Он слишком умный для этого! 🤖", "— Сколько программистов нужно, чтобы вкрутить лампочку? — Ни одного, это аппаратная проблема! 💡"]
    await update.message.reply_text(f"😄 <b>Шутка дня:</b>\n\n{random.choice(jokes)}", parse_mode=ParseMode.HTML)

async def quote_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    quotes = ["Единственный способ сделать великую работу — любить то, что вы делаете. — Стив Джобс", "Инновация отличает лидера от последователя. — Стив Джобс", "Программирование — это искусство превращать кофе в код. — Неизвестный автор", "Лучший код — это отсутствие кода. — Джефф Етвуд", "Сначала сделай это, потом сделай правильно, потом сделай лучше. — Адди Османи"]
    await update.message.reply_text(f"💭 <b>Цитата:</b>\n\n<i>{random.choice(quotes)}</i>", parse_mode=ParseMode.HTML)

async def fact_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    facts = ["🌍 Земля — единственная планета Солнечной системы, названная не в честь бога.", "🐙 У осьминогов три сердца и голубая кровь.", "🍯 Мёд — единственный продукт питания, который не портится тысячи лет.", "💎 Алмазы формируются на глубине около 150 км под землёй.", "🧠 Человеческий мозг потребляет около 20% всей энергии тела.", "🌊 95% мирового океана остаются неисследованными.", "⚡ Молния в 5 раз горячее поверхности Солнца.", "🦈 Акулы существуют дольше, чем деревья — более 400 миллионов лет!"]
